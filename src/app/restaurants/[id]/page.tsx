...
import { fetchRestaurantById } from "@/lib/apiService";
  ...
  if (loading) {
    return (
      <div className="text-center py-10 sm:py-20">
        <h1 className="font-headline text-3xl sm:text-4xl mb-4">Loading Restaurant...</h1>
        {/* You could add a spinner or other loading indicator here */}
      </div>
    );
  }
    ...
    return (
      <div className="space-y-8 sm:space-y-10">
        ...
        <div>
          ...
          {restaurant.menuItemList && restaurant.menuItemList.length > 0 ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 sm:gap-6">
              {restaurant.menuItemList.map((item) => (
